...
parser.add_argument('--useMean', action='store_true', dest='useMean', help='faz o programa usar a média das bounds das imagens passadas na especificação para definir a range da segmentação.')
parser.add_argument('--doNot', action='store_true', dest='useMedian', help='faz o programa usar a mediana de cada coordenada das bounds das imagens passadas na especificação para definir a range da segmentação.')
...
if args.useMean:
    hsv_lowerbound = np.array([0, 0, 0])
    hsv_upperbound = np.array([0, 0, 0])

    for filename in hsv_bounds:
        hsv_lowerbound = np.add(hsv_lowerbound, hsv_bounds[filename]['lower'])
        hsv_upperbound = np.add(hsv_upperbound, hsv_bounds[filename]['upper'])

    hsv_lowerbound = np.multiply(hsv_lowerbound, 1.0/len(hsv_bounds))
    hsv_upperbound = np.multiply(hsv_upperbound, 1.0/len(hsv_bounds))
